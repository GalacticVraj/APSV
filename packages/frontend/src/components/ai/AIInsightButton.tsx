...
export default function AIInsightButton({
  ...
  const fetchInsight = useCallback(async (forceRefresh = false) => {
    ...
    try {
      const { data } = await apiClient.post('/api/ai/insights', {
        templateKey,
        dataPackage,
      });

      if (data.error) {
        setPanel({ phase: 'error', message: data.error });
        return;
      }
  ...
  return (
    <div ref={wrapperRef} className="relative inline-flex" id={`${id}-wrapper`}>
      ...
      <button
        ...
        className={`
          inline-flex items-center justify-center w-6 h-6 rounded-sm border transition-all duration-150
          focus:outline-none focus:ring-2 focus:ring-forest-700 focus:ring-offset-1
          ${isError
            ? 'border-red-300 text-red-600 bg-red-50 hover:bg-red-100'
            : isOpen
              ? 'border-forest-300 text-forest-700 bg-forest-50'
              : 'border-transparent text-forest-700 hover:border-forest-200 hover:bg-forest-50'
          }
        `}
      >
        {isLoading ? (
